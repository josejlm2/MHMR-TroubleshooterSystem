class Ticket < ActiveRecord::Base
 ...
 scope :location, ->(location){ where("location_id = ?", Location.find_by_name(location).id)}
 scope :category, ->(category){ where("category_id = ?", Category.find_by_name(category).id)}
